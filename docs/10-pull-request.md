...
> 브랜치 전략·커밋 컨벤션: [09-team.md 2. 팀 운영 규칙](09-team.md#2-팀-운영-규칙)
> 코드 병합은 PR 로만 한다 (mission 4-7절 "PR 기반 Merge 기록", 6절 "브랜치/PR 기록이 저장소에 남아야 한다").
> **예외: `docs/` 문서만 바꾸는 커밋은 develop 에 직접 push** 한다 (팀 합의 — [09-team.md 2-1. 브랜치](09-team.md#2-1-브랜치)).
> 아래 본문 템플릿은 `.github/pull_request_template.md` 에도 등록되어 PR 생성 시 자동으로 채워진다.

## 1. PR 규칙

### 1-1. 제목
커밋 컨벤션과 같은 형식: `<type>(<scope>): <변경 요약> #<이슈번호>`
scope 는 **`be`(백엔드) · `fe`(프론트) 두 가지만** 사용하고, 어느 쪽도 아닌 작업은 scope 없이 쓴다.
이슈 번호는 **`#1` 형식(앞자리 0 없음)** — 확정 ([09-team.md 2-2. 커밋 — "유의미한 커밋" 기준](09-team.md#2-2-커밋--유의미한-커밋-기준))
```
feat(fe): 로그인 화면과 토큰 저장 추가 #6
fix(fe): 한글 입력 중 Enter 중복 전송 수정 #14
hotfix(be): 운영 CORS 허용 도메인 누락 수정 #21
docs: v1.7 update
```

### 1-2. 대상(base) 브랜치

**브랜치 보호 규칙 (팀 합의 — [09-team.md 2-1. 브랜치](09-team.md#2-1-브랜치))**: `main`·`develop` 모두 **PR 필수 + 승인 1명**.
예외는 하나뿐이다 — **`docs/` 문서만 수정하는 커밋은 develop 에 직접 push** (보호 규칙에 팀원 bypass 지정).
...
| 항목 | 규칙 |
|------|------|
| 리뷰어 | 최소 **1명 승인** 필수 (2인 상호 리뷰: 성원모 PR → 이성준, 이성준 PR → 성원모) |
| 리뷰 기한 | 요청 후 24시간 이내 1차 코멘트 |
| 머지 조건 | 승인 1 + 테스트 통과 + 충돌 없음 + 체크리스트 완료 |
| 머지 주체 | **PR 작성자**가 승인 후 직접 머지 |
| 머지 방식 | **Create a merge commit (확정)** — Squash·Rebase 금지 → 개인 커밋과 머지 기록이 모두 남아야 함 (mission 4-7절). 저장소 설정에서 merge commit 만 허용 |
...
- mission: 2-2절 관리자/내부 로그 확인 화면, 4-4절 관리자 조회 API/화면
- features.md: F10
...
| 체크리스트 | 민감정보 커밋(mission 6절)·테스트 누락을 머지 전에 차단 |
| 머지 방식 = Merge commit | Squash 시 개인 커밋이 1개로 합쳐져 **팀원별 커밋 10회**(mission 4-7절) 집계에 불리 |
