...
| **L2 API 흐름** | curl / httpie 스크립트 | 가입→로그인→질문→응답→로그 전 흐름 | 팀장 |
| **L3 브라우저** | 수동 + (여유 시) Playwright | 화면 조작, 리다이렉트, 오류 표시, 반응형 | 이성준 |
| **L4 배포/외부망** | 휴대폰 데이터망에서 배포 URL 접속 | mission §4-6 | 팀장 |
...
| ID | 테스트 | 기대 | mission |
|----|--------|------|---------|
| V01 | 회원가입 | `201`, 응답에 `hashed_password` 없음 | §4-2 |
| V02 | 같은 이메일 재가입 | `409 EMAIL_ALREADY_EXISTS` | §4-2 |
| V03 | 비밀번호 7자 / 잘못된 이메일 / 닉네임 21자 | `422 VALIDATION_ERROR` | §4-5 |
| V04 | DB 의 `hashed_password` 확인 | bcrypt prefix(`$2b$`), 평문 아님 | §4-2 |
| V05 | 로그인 성공 | `200`, `access_token`·`token_type=bearer`·`expires_in=3600` | §4-2 |
| V06 | 틀린 비밀번호 / 없는 이메일 | `401 INVALID_CREDENTIALS` (메시지 동일) | §4-2 |
| V07 | `GET /api/auth/me` 유효 토큰 | `200 {id, email, nickname}` | §4-2 |
| V08 | 토큰 없음 / 변조 / 만료 | `401 UNAUTHORIZED` | §4-2 |
| V09 | 비로그인 `POST /api/chat`, `GET /api/me/chats` | `401 UNAUTHORIZED` | §4-2 |
| V10 | 챗 성공 | `200 {chat_id, question, answer, created_at}`, DB 1건 증가 | §4-3, §4-4 |
| V11 | 두 번째 질문 | 프롬프트에 직전 Q/A 포함 (AI 클라이언트를 목으로 두고 payload 검사) | §4-3 |
| V12 | 7번째 질문 | 컨텍스트가 `AI_CONTEXT_TURNS`(5)턴으로 제한됨 | §4-3 |
| V13 | 빈 문자열 / 공백만 | `422 VALIDATION_ERROR` | §4-5 |
| V14 | 1001자 → 422 / 1000자 → 200 (경계값) | | §4-5 |
| V15 | AI 타임아웃 강제 (`httpx.TimeoutException`) | `504 AI_TIMEOUT`, **서버 프로세스 유지** | §4-5, §6 |
| V16 | AI 500/429/연결 실패 강제 | `502 AI_CALL_FAILED` | §4-5 |
| V17 | 장애 직후 정상 질문 | `200` (서비스 유지) | §4-5 |
| V18 | 사용자 A 의 로그가 B 에게 안 보임 | `items` 에 타인 기록 없음 | §4-4 |
| V19 | `limit=101` / `offset` 동작 | 상한 처리, 최신순, `total` 정확 | §4-4 |
| V20 | 로그 이벤트 | `request_received`·`ai_call_start`·`ai_call_success`/`ai_call_failed`·`db_save_success` 존재, **비밀번호·API 키 미기록** | §4-5 |
| V21 | DB commit 실패 강제 | `db_save_failed` 로그 남고 서버 유지 | §4-5 |
| V22 | CORS preflight (`OPTIONS`) | 허용 Origin 은 200, 미허용은 차단 | 배포 |
...
curl -s -o /dev/null -w '%{http_code} signup\n' -X POST $BASE/api/auth/signup \
  ...
  -d "{\"email\":\"$EMAIL\",\"password\":\"password1234\",\"nickname\":\"e2e\"}"   # 201

# 2. 중복 가입
curl -s -o /dev/null -w '%{http_code} duplicate\n' -X POST $BASE/api/auth/signup \
  -H 'Content-Type: application/json' \
  -d "{\"email\":\"$EMAIL\",\"password\":\"password1234\",\"nickname\":\"e2e\"}"   # 409

# 3. 비로그인 차단
curl -s -o /dev/null -w '%{http_code} chat-no-auth\n' -X POST $BASE/api/chat \
...
# 5. 내 정보
curl -s -o /dev/null -w '%{http_code} me\n' -H "Authorization: Bearer $TOKEN" $BASE/api/auth/me   # 200
...
# 7. 입력 검증
curl -s -o /dev/null -w '%{http_code} empty\n' -X POST $BASE/api/chat \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{"message":"   "}'   # 422

# 8. 로그 조회
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/me/chats?limit=20&offset=0"                 # total/items

# 9. 잘못된 토큰
curl -s -o /dev/null -w '%{http_code} bad-token\n' -H "Authorization: Bearer xxx" $BASE/api/auth/me  # 401
```

체크: `201 / 409 / 401 / 200 / 문맥 반영 / 422 / total≥2 / 401`
...
| B05 | 틀린 비밀번호 | "이메일 또는 비밀번호가 올바르지 않습니다.", 비밀번호 칸만 비움 |
| B06 | 로그인 성공 | `/chat` 이동, 헤더에 닉네임·탭 표시 |
...
| B12 | AI 실패 재현 (키를 잘못된 값으로 두고 재배포) | 오류 말풍선 + 코드 표시, 서버 유지 |
| B13 | 새로고침 | 로그인 유지(`/auth/me`), 이전 대화 복원 |
| B14 | 내 대화 로그 | 카드 수 = 질문 수, `total` 표시 |
| B15 | 새 계정의 로그 화면 | "아직 저장된 대화가 없습니다." |
| B16 | 로그아웃 → 뒤로가기 | `/login` 유지 (보호 페이지 재진입 차단) |
| B17 | 토큰 만료 후 요청 | 로그인 화면으로 이동 |
| B18 | 375px 폭 | 가로 스크롤 0, 1열 |
| B19 | 백엔드 슬립 상태에서 첫 요청 | 로딩 유지 후 정상 응답 (콜드 스타트 안내) |

### 1-4. L4 — 배포 / 외부망

| ID | 확인 | 기대 |
|----|------|------|
| D01 | 휴대폰 데이터망에서 Vercel URL 접속 | 화면 로드 |
...
| L3 브라우저 | ⬜ 미실행 | 스펙 구현 후 |
| L4 배포/외부망 | ⬜ 미실행 | Render·Vercel 배포 후 |
...
| 로그아웃 후 이전 쿠키 재사용 차단 | ❌ **폐기** — 서버 로그아웃 API 없음. 토큰 만료 테스트로 대체 |
| 아이디(username) 형식 검증 | 🔁 **수정** — 이메일 형식 + 닉네임 길이로 변경 |
| 입력 검증 400 `INVALID_INPUT` | 🔁 **수정** — 422 `VALIDATION_ERROR` |
| AI 타임아웃/오류 503 `AI_TIMEOUT`/`AI_ERROR` | 🔁 **수정** — 504 `AI_TIMEOUT` / 502 `AI_CALL_FAILED` |
| 로그 응답 `{items, count, avg_latency_ms}` | 🔁 **수정** — `{total, items}` + `offset` |
| Nginx SPA fallback / 프록시 / Secure 쿠키 | 🔁 **대체** — Vercel rewrite + Render CORS 확인으로 |
...
| 3 | 백엔드 중단 시 "요청을 처리하지 못했습니다" — 원인 불명확 | JSON 본문 없는 게이트웨이 오류를 일반 오류로 처리 | 502/503/504 + 본문 없음 → "서버에 연결할 수 없습니다" |
