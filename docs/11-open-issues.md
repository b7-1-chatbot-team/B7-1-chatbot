...
| # | 항목 | 위치 A | 위치 B | 결정 |
|---|------|--------|--------|------|
| C1 | 팀원 이름 | "어썸체크(팀장) · 이성준 · 박성현A" (09 §0·§3·§4·§6, 03 예시 닉네임) | 성원모 · 박성현 · 이성준 (`mission.md:7`, 10 §1-4, 08 §2) | **실명으로 확정** — 성원모(팀장) · 박성현 · 이성준. 09 문서 전체 반영 완료. `03-api.md` 등의 "어썸체크"는 **API 예시 닉네임 값**이라 변경 대상 아님 |
| C2 | 역할 분담 | 성원모 = 인증·DB·인프라, 박성현 = AI, 이성준 = 프론트 전담 (09 §3) | ~~handoff.md 버전~~ (파일 삭제됨) | **09 §3 버전으로 확정** — 성원모=인증·DB·인프라, 박성현=AI 파이프라인, 이성준=프론트엔드 전체(F1~F14, 관리자 화면 포함). 백엔드 관리자 API(B15~B19)는 G7 에서 박성현으로 확정 |
| C4 | 커밋 메시지 형식 | `type(scope): 내용`, 타입 7개, 이슈 번호 없음 (09 §2-2) | `type: 제목 #1`, 타입 8개(hotfix 포함) (10 §1-1) | **확정** — `type(scope): 내용 #이슈번호`, scope 는 **`be`·`fe` 두 가지만**(해당 없으면 생략), 타입 8종(hotfix 포함), 설명할 내용이 있으면 본문(디스크립션) 작성. 09 §2-2 · 10 §1-1 반영 완료 |
| C5 | 브랜치 접두어 | feature·fix·docs·chore 4종 (09 §2-1) | + refactor·test, hotfix 는 main 분기 (10 §1-2) | **7종 모두 사용 확정** — feature·fix·refactor·test·docs·chore(develop 분기) + hotfix(main 분기). 09 §2-1 에 누락분 반영 완료 |
| C6 | 기능 번호 체계 | B1~B16 · A1~A12 · F1~F18 · C1~C5 (09 §3·§4) | B1~B19 · F1~F14 · C1~C4 (features.md, 08 §2) | |
| C8 | 리뷰 순환 순서 | 어썸체크 → 박성현A → 이성준 (09 §3) | 성원모 → 박성현 → 이성준 (10 §1-4) | **확정** — 성원모 → 박성현 → 이성준 → 성원모 (C1 실명 확정에 따라 두 문서 표기 일치) |
| C9 | 브랜치 보호 설정 | 승인 1명 필수 (09 §2-1) | docs 직접 push 허용하려면 develop 보호 규칙 bypass 필요 | **확정** — `main`·`develop` 모두 PR 필수 + 승인 1명, `docs/` 문서만 수정하는 커밋은 develop 직접 push(보호 규칙 bypass 지정). 09 §2-1 반영 완료, **GitHub 설정은 저장소 관리자가 적용 필요** |
...
| F7 | ~~develop 보호 규칙에서 docs 직접 push 를 어떻게 허용할 것인가~~ → **bypass 대상 지정으로 확정(C9)**. 남은 것은 GitHub 에 실제 설정 적용 | GitHub 설정, 09 §2-1 |
...
| G7 | **담당자** | 미정 — 백엔드 B15~B19, 프론트 F10~F14 (C2 와 함께 결정) | **프론트 F10~F14 = 이성준 (확정)** · **백엔드 B15~B19 = 박성현 (확정)** — 관리자 API 5종 + 관리자 조회 CRUD. `require_admin`·관리자 시드는 성원모(인증 트랙) 제공 |
